...
import { streamChat, type StreamChatOptions } from "./chat.js";
...
interface CommonOpts {
  model?: string;
  thinking?: string; // commander gives strings — we parse
...
function parseModelOpts(opts: CommonOpts): StreamChatOptions {
  const out: StreamChatOptions = {};
  if (opts.model) out.model = opts.model;
  if (opts.thinking !== undefined) {
    if (opts.thinking === "true") out.thinking = true;
    else if (opts.thinking === "false") out.thinking = false;
    else {
  ...
  if (opts.reasoningEffort) {
    if (!validateReasoningEffort(opts.reasoningEffort)) {
      throw new Error(
        `--reasoning-effort must be low/medium/high, got: ${opts.reasoningEffort}`,
      );
    }
  ...
  if (opts.showThinking) out.showThinking = true;
  if (opts.system) out.systemPrompt = opts.system;
  if (opts.temperature) {
...
function addModelOpts(cmd: Command): Command {
  return cmd
    ...
    .option(
      ...
      "Reasoning effort when thinking is on (low/medium/high)",
    )
...
addModelOpts(
  program
    ...
    .option("--temperature <n>", "Sampling temperature (default: 0.7)"),
).action(async (prompt: string | undefined, opts: CommonOpts) => {
...
addModelOpts(
  program
    ...
    .description("Fetch a URL and summarize/extract with deepseek (streams)"),
).action(async (url: string, prompt: string | undefined, opts: CommonOpts) => {
  const promptText = await resolvePrompt(prompt);
  const modelOpts = parseModelOpts(opts);

  const page = await fetchAndExtract(url, config.webFetch);
  process.stderr.write(
    `[fetched ${page.charsExtracted} chars in ${page.fetchMs + page.parseMs}ms]\n`,
  );

  const fullPrompt = `Given this web page content:\n\n${page.content}\n\n---\n\n${promptText}`;
  const effective: StreamChatOptions = {
    ...modelOpts,
...
addModelOpts(
  program
    ...
    .option("-n, --max-results <n>", "Max search results to synthesize", "10"),
).action(
  async (
    query: string,
    opts: CommonOpts & { maxResults?: string },
  ) => {
    const modelOpts = parseModelOpts(opts);
