...
import type { ModelCallParams, AgentCallParams } from "./types.js";
...
const MODEL_PARAM_SCHEMA = {
  model: {
    type: "string",
    description:
      "DeepSeek model to use. Tool-specific defaults apply when omitted.",
    enum: config.model.allowed,
  },
  max_tokens: {
    type: "integer",
    description:
      "Max output tokens. Omit to let the API decide (v4 supports up to 384K).",
  },
  thinking: {
    type: "boolean",
    description:
      "Enable/disable thinking mode. Omit to use API default (on for v4 models). " +
      "Set false for faster/cheaper responses on simple tasks.",
  },
  reasoning_effort: {
    type: "string",
    description:
      "How hard the model should think (only meaningful when thinking is on).",
 ...
 */
function extractModelParams(args: Record<string, unknown> | undefined): ModelCallParams {
  if (!args) return {};
  const params: ModelCallParams = {};
  if (typeof args.model === "string") params.model = args.model;
  if (typeof args.max_tokens === "number") params.maxTokens = args.max_tokens;
  if (typeof args.thinking === "boolean") params.thinking = args.thinking;
    ...
    tools: [
      {
        name: "deepseek_agent",
        description:
          "SLOW: Agentic loop. Use when task requires file system, bash, or web access. " +
        ...
        inputSchema: {
          ...
          properties: {
            ...
            strict_tools: {
              type: "boolean",
              description:
                "Enable DeepSeek strict tool-calling mode (Beta). " +
      ...
      },
      {
        name: "deepseek_chat",
        description:
          "PREFERRED: Single-turn chat completion. Use for analysis, summarization, " +
      ...
      },
      {
        name: "web_fetch",
        description:
          "PREFERRED: Fetch a web page and extract information. " +
      ...
      },
      {
        name: "web_fetch_raw",
        description:
          "Fetch a web page and return raw extracted text. No AI processing. " +
      ...
      },
      {
        name: "web_search",
        description:
          "Search the web (Brave) and synthesize results with DeepSeek. " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "deepseek_agent") {
      ...
      const agentParams: AgentCallParams = {
        ...modelParams,
