    ...
    tools: [
      {
        name: "deepseek_agent",
        description:
          "SLOW: Only use when task requires file system or bash access. " +
      ...
      },
      {
        name: "deepseek_chat",
        description:
          "PREFERRED: Fast, cheap chat completion. Use for analysis, summarization, writing, code review, " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "deepseek_agent") {
      ...
      const model = (args?.model as string) ?? config.model.default;
      const maxIterations =
        (args?.max_iterations as number) ?? config.agent.maxIterations;
      const timeoutSeconds =
        (args?.timeout_seconds as number) ?? config.agent.timeoutSeconds;
    ...
    if (name === "deepseek_chat") {
      ...
      const maxTokens = (args?.max_tokens as number) ?? 8192;
